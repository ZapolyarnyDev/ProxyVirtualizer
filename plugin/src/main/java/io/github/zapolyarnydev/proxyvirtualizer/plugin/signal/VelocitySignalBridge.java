...
public final class VelocitySignalBridge {
  ...
  private void installPacketTap(Player player) {
    ...
    TapRegistration tapRegistration =
        new TapRegistration(
    ...
    try {
      ...
      boolean decodedInstalled =
          installDecodedPacketTap(player, pipeline, tapRegistration.decodedTapName());
      installedTapNames.put(
          playerId, tapRegistration.withInstalled(rawInstalled, decodedInstalled));
      ...
      installedTapNames.remove(playerId);
      logger.debug(
          "Unable to install signal packet taps for player {}", player.getUsername(), exception);
    }
  }
  ...
  private void uninstallPacketTap(Player player) {
    ...
    try {
      ...
      logger.debug(
          "Unable to remove signal packet taps for player {}", player.getUsername(), exception);
    }
  }

  private boolean installRawPacketTap(Player player, Object pipeline, String tapName)
  ...
  private boolean installDecodedPacketTap(Player player, Object pipeline, String tapName)
      throws ReflectiveOperationException {
    if (getPipelineHandler(pipeline, tapName) != null) {
      return true;
    }
  ...
  private Object createInboundTapHandler(Player player, InboundInspector inspector)
      throws ClassNotFoundException {
    ClassLoader classLoader = player.getClass().getClassLoader();
    Class<?> channelInboundHandlerClass =
        Class.forName("io.netty.channel.ChannelInboundHandler", true, classLoader);
          ...
          try {
            return switch (methodName) {
              ...
              case "channelWritabilityChanged" ->
                  forwardInboundEvent(args[0], "fireChannelWritabilityChanged");
    ...
    return java.lang.reflect.Proxy.newProxyInstance(
        classLoader, new Class<?>[] {channelInboundHandlerClass}, invocationHandler);
  }

  private Object handleObjectMethod(
      Object proxy, String methodName, Method method, Object[] args, Player player) {
    return switch (methodName) {
  ...
  private void inspectRawInboundMessage(Player player, Object message) {
    ...
    try {
      ...
      switch (packetId) {
        case PACKET_ID_SET_PLAYER_POSITION_1_21_4 ->
            publishPosition(player, copy, PlayerPacketSignalKind.POSITION);
        case PACKET_ID_SET_PLAYER_POSITION_AND_ROTATION_1_21_4 ->
            publishPositionAndRotation(player, copy);
        case PACKET_ID_SET_PLAYER_ROTATION_1_21_4 ->
            publishRotation(player, copy, PlayerPacketSignalKind.ROTATION);
        default -> {}
      }
    } catch (ReflectiveOperationException | RuntimeException exception) {
      logger.debug(
          "Failed to inspect inbound packet for player {}", player.getUsername(), exception);
    }
  }
  ...
  private boolean inspectDecodedChatOrCommandPacket(Player player, Object message) {
    ...
    try {
      if (className.endsWith("SessionPlayerChatPacket")
          || className.endsWith("KeyedPlayerChatPacket")) {
      ...
      logger.debug(
          "Failed to inspect decoded chat/command packet for player {}",
          player.getUsername(),
          exception);
    }
  ...
  private boolean inspectDecodedMovementPacket(Player player, Object message) {
    ...
    try {
      ...
      Float yaw =
          firstNonNull(
              tryInvokeFloatGetter(message, "getYaw"), tryInvokeFloatGetter(message, "getYRot"));
      Float pitch =
          firstNonNull(
              tryInvokeFloatGetter(message, "getPitch"), tryInvokeFloatGetter(message, "getXRot"));
      ...
      if (hasPosition && hasRotation) {
        ...
        signalBus.publish(
            new PlayerLookSignal(
                player,
                new PlayerLookPayload(
                    yaw,
                    pitch,
                    isOnGround(flags),
                    hasHorizontalCollision(flags),
      ...
      logger.debug(
          "Failed to inspect decoded movement packet for player {}",
          player.getUsername(),
          exception);
      return false;
    }
  ...
  private void publishCommandFromRaw(
      Player player, String rawCommand, String invocationSource, String signedState) {
    CommandParts parts = splitCommand(rawCommand);
    signalBus.publish(
        new PlayerCommandSignal(
            player,
            new PlayerCommandPayload(
                rawCommand,
                parts.label(),
  ...
  private void publishPosition(Player player, Object byteBuf, PlayerPacketSignalKind kind)
    ...
    signalBus.publish(
        new PlayerMoveSignal(
            player,
            new PlayerMovePayload(
                x, y, z, isOnGround(flags), hasHorizontalCollision(flags), kind)));
  }
  ...
  private void publishRotation(Player player, Object byteBuf, PlayerPacketSignalKind kind)
    ...
    signalBus.publish(
        new PlayerLookSignal(
            player,
            new PlayerLookPayload(
                yaw, pitch, isOnGround(flags), hasHorizontalCollision(flags), kind)));
  }
  ...
  private static boolean isByteBuf(Object value) {
    ...
    return "io.netty.buffer.ByteBuf".equals(className)
        || inheritsFrom(value.getClass(), "io.netty.buffer.ByteBuf");
  }
  ...
  private static Double tryInvokeDoubleGetter(Object target, String methodName)
      throws ReflectiveOperationException {
    try {
      Object result = target.getClass().getMethod(methodName).invoke(target);
  ...
  private static Float tryInvokeFloatGetter(Object target, String methodName)
      throws ReflectiveOperationException {
    try {
      Object result = target.getClass().getMethod(methodName).invoke(target);
  ...
  private static Boolean tryInvokeBooleanGetter(Object target, String methodName)
      throws ReflectiveOperationException {
    try {
      Object result = target.getClass().getMethod(methodName).invoke(target);
  ...
  private static int readVarInt(Object byteBuf) throws ReflectiveOperationException {
    ...
    do {
      Number readValue =
          (Number) invokeByteBufMethod(byteBuf, "readByte", new Class<?>[0], new Object[0]);
  ...
  private static int readUnsignedByte(Object byteBuf) throws ReflectiveOperationException {
    Number value =
        (Number) invokeByteBufMethod(byteBuf, "readUnsignedByte", new Class<?>[0], new Object[0]);
  ...
  private static Object invokeByteBufMethod(
      Object byteBuf, String methodName, Class<?>[] parameterTypes, Object[] args)
      throws ReflectiveOperationException {
  ...
  private static Object getPipelineHandler(Object pipeline, String name)
      throws ReflectiveOperationException {
  ...
  private static boolean addLast(Object pipeline, String name, Object handler)
      throws ReflectiveOperationException {
    try {
      Class<?> channelHandlerClass =
          Class.forName(
              "io.netty.channel.ChannelHandler", true, handler.getClass().getClassLoader());
      pipeline
          .getClass()
  ...
  private static void removePipelineHandler(Object pipeline, String name)
      throws ReflectiveOperationException {
    try {
  ...
  private static void invokePipelineMutation(
      Object pipeline, String methodName, String baseName, String name, Object handler)
      throws ReflectiveOperationException {
    Class<?> channelHandlerClass =
        Class.forName("io.netty.channel.ChannelHandler", true, handler.getClass().getClassLoader());
    pipeline
        .getClass()
  ...
  private static Object invokeContext(
      Object context, String methodName, Class<?>[] parameterTypes, Object[] args)
      throws NoSuchMethodException, InvocationTargetException, IllegalAccessException {
    ClassLoader classLoader = context.getClass().getClassLoader();
    try {
      Class<?> channelHandlerContextClass =
          Class.forName("io.netty.channel.ChannelHandlerContext", true, classLoader);
  ...
  private record TapRegistration(
      String rawTapName, String decodedTapName, boolean rawInstalled, boolean decodedInstalled) {
