...
public final class VelocityVirtualPacketSender {
  ...
  private static final String PROTOCOL_UTILS_CLASS =
      "com.velocitypowered.proxy.protocol.ProtocolUtils";
  private static final String DIMENSION_INFO_CLASS =
      "com.velocitypowered.proxy.connection.registry.DimensionInfo";
  private static final String RESPAWN_PACKET_CLASS =
      "com.velocitypowered.proxy.protocol.packet.RespawnPacket";
  private static final String COMPOUND_BINARY_TAG_CLASS =
  ...
  public boolean sendTitle(
      VirtualServer virtualServer, Player player, Component title, Component subtitle) {
  ...
  public boolean bootstrapVoidLimbo(VirtualServer virtualServer, Player player) {
    ...
    try {
      ...
      Object netherRespawn =
          createRespawnPacket(
              player.getProtocolVersion(), NETHER_DIMENSION_ID, "minecraft:the_nether", false);
      Object overworldRespawn =
          createRespawnPacket(
              player.getProtocolVersion(), OVERWORLD_DIMENSION_ID, "minecraft:overworld", true);
  ...
  private boolean canSend(VirtualServer virtualServer, Player player, String packetKey) {
    ...
    boolean inTargetVirtualServer =
        connectionStorage.getVirtualServer(player).map(virtualServer::equals).orElse(false);
  ...
  private Object createRespawnPacket(
      ProtocolVersion protocolVersion, int dimensionId, String levelName, boolean flatWorld)
      throws ReflectiveOperationException {
    ClassLoader classLoader = getClass().getClassLoader();

    Class<?> dimensionInfoClass = Class.forName(DIMENSION_INFO_CLASS, true, classLoader);
    Constructor<?> dimensionInfoConstructor =
        dimensionInfoClass.getConstructor(
            String.class, String.class, boolean.class, boolean.class, ProtocolVersion.class);
    Object dimensionInfo =
        dimensionInfoConstructor.newInstance("", levelName, flatWorld, false, protocolVersion);
    ...
    Constructor<?> respawnConstructor =
        respawnPacketClass.getConstructor(
            int.class,
            long.class,
            short.class,
            short.class,
  ...
  private void sendStartWaitingForLevelChunksGameEvent(
      VirtualServer virtualServer, Player player, Object connection)
      throws ReflectiveOperationException {
    ClassLoader classLoader = getClass().getClassLoader();
    ...
    int packetId =
        virtualServer
            .getPacketVersion(
                VirtualPacketKeys.GAME_EVENT, player.getProtocolVersion().getProtocol())
    ...
    byteBufClass
        .getMethod("writeByte", int.class)
        .invoke(byteBuf, GAME_EVENT_START_WAITING_FOR_LEVEL_CHUNKS);
    byteBufClass.getMethod("writeFloat", float.class).invoke(byteBuf, 0.0F);
  ...
  private void sendSynchronizePlayerPosition(
    ...
    int packetId =
        virtualServer
            .getPacketVersion(VirtualPacketKeys.PLAYER_POSITION, protocol)
            .map(VirtualServer.PacketVersionRule::packetVersion)
